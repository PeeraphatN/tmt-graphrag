...
def _fallback_substances_from_fsn(fsn: str) -> str | None:
    if not fsn:
        return None
...
def extract_structured_data(results: dict, question_type: str) -> dict:
    ...
    for node in candidate_nodes:
        ...
        if fsn:
            ...
            # formula/substance fallback
            if question_type in ("formula", "substance") and "substances_fallback" not in entity:
                ing = _fallback_substances_from_fsn(fsn)
                if ing:
                    entity["substances_fallback"] = ing
    ...
    return {
        ...
        "entities": entities,
        # Keep relation evidence so the LLM can reason over graph links, not only node fields.
        "evidence": evidence,
