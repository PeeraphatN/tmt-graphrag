...
# Configuration
TRAIN_PATH = DATA_DIR / "benchmark_train.json"
TEST_PATH = DATA_DIR / "benchmark_test.json"
...
INTENTS = {
    ...
    "substance_find": "Find active substances, components, formula details, or chemical names.",
    "substance_check": "Check if a drug contains specific substances (e.g. alcohol, steroids, peanut).",
    "substance_count": "Count number of substances, substances, or active components.",
...
def run_benchmark():
    ...
    for model in MODELS:
        logger.log(f"\n{'='*50}")
        logger.log(f" ðŸ¤– Evaluation: {model}")
        logger.log(f"{'='*50}")

        # Fetch Model Info
        model_info = get_model_info(model)
        ctx_window = model_info['context_window']
        logger.log(f"   â„¹ï¸ Context Window: {ctx_window}")
    ...
    # Summary Table
    logger.log(f"\n{'='*100}")
    logger.log(f" ðŸ“Š FINAL LLM BENCHMARK SUMMARY")
    logger.log(f"{'='*100}")
