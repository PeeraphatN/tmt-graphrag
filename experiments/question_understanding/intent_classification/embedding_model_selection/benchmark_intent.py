...
def get_ollama_embeddings(texts, model_name):
    ...
    for i, text in enumerate(texts):
        ...
        try:
            ...
            print(f"\nÃ¢ÂÅ’ Error fetching embedding for '{text}': {e}")
            raise e
...
def save_results_to_file(content):
    ...
    print(f"\nÃ¢Å“â€¦ Results saved to: {filepath}")

def run_model_benchmark(model_name, X_train_text, y_train, X_test_text, y_test, logger):
    logger.log(f"\n{'='*40}")
    logger.log(f" Ã°Å¸Â§Â  Benchmarking Model: {model_name}")
    logger.log(f"{'='*40}")
    ...
    try:
        ...
        logger.log(f"\nÃ¢ÂÅ’ Failed to connect to Ollama. Ensure 'ollama serve' is running and '{model_name}' is pulled.")
...
def run_benchmark():
    ...
    # Load Data
    logger.log(f"Ã°Å¸â€œâ€š Loading Data...")
    ...
    # Final Summary Table
    logger.log(f"\n{'='*180}")
    logger.log(f" Ã°Å¸â€œÅ  FINAL COMPARISON SUMMARY")
    logger.log(f"{'='*180}")
