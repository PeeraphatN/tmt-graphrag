...
- `topics_intents` (multi-label): `manufacturer`, `ingredient`, `nlem`, `formula`, `hierarchy`, `id_lookup`
...
- Dataset: `src/api/intent_dataset.json`
- Labels: 18 (`6 topics x 3 actions`)
- Samples: 270 total
- Actions represented: `find`, `check`, `count` only
- Topics represented: `manufacturer`, `ingredient`, `nlem`, `hierarchy`, `formula`, `general`
...
Dataset and source:
- Source file: `src/api/intent_dataset.json`
- Derived HIC reference file: `experiments/intent_benchmarks/intent_dataset_v2_reference.json`
...
- Comparison artifact: `experiments/intent_benchmarks/results/real_query_fci_vs_hic_preview.json`
...
|---|---|---|---|---|
| `พาราอยู่ในบัญชียาหลักใช่หรือไม่` | `verify + nlem` | `lookup + nlem` | `verify + nlem` | HIC aligns better with yes/no verification intent. |
| `Paracetamol อยู่ในบัญชียาหลักใช่หรือไม่` | `verify + nlem` | `lookup + nlem` | `verify + nlem` | Same pattern in English/Thai mixed form; HIC is more action-correct. |
| `MACROPHAR ผลิตยาพาราไหม` | `verify + manufacturer` | `lookup + manufacturer` | `verify + manufacturer` | HIC captures verification cue from "ไหม" more reliably. |
| `paracetamal กี่ ml เป็นอันตราย` | likely safety/unknown (not simple count) | `lookup + formula` | `count + formula` | Both are weak here; action space lacks safety-risk intent. |
| `สวัสดี ผมชื่ออะไรให้ทาย` | `unknown` (OOD/chit-chat) | `verify + general` | `lookup + general` | Both fail as expected because `unknown` class is not modeled yet. |
| `ขอ TMTID ของยา cefazolin ...` | `lookup + id_lookup` | `lookup + hierarchy` | `lookup + hierarchy` | Both miss `id_lookup` topics because this topics is absent in current label space. |
...
- `topics_intents` (multi-label):
  - `manufacturer`, `ingredient`, `nlem`, `formula`, `hierarchy`, `id_lookup`
